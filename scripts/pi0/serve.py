...
def main() -> None:
    ...
    latent_group.add_argument(
        ...
        action="store_true",
        help=(
            "return SAFE pre-velocity endpoint tokens using concat-2 over "
            "diffusion steps and action horizon (4096 floats)"
        ),
    )
    ...
    if safe_pre_velocity_latent:
        ...
        if safe_pred_horizon < 2:
            raise ValueError("SAFE concat-2 requires an action horizon of at least two")
        policy_metadata.update(
