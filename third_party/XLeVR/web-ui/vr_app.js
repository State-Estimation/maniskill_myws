...
AFRAME.registerComponent('controller-updater', {
  init: function () {
    ...
    // ==========================================
    // 👇 重点修改部分：WebSocket 连接逻辑 👇
    // ==========================================
    
    // 获取我们在 index.html 里加的调试球
    const statusSphere = document.querySelector('#statusSphere');

    // 强制使用 localhost (配合 ADB Reverse)
    ...
    try {
      ...
      this.websocket.onopen = (event) => {
        ...
        // ✅ 连接成功 -> 变绿
        if (statusSphere) statusSphere.setAttribute('color', '#00ff00'); // Green
        
        // 发送测试消息，触发 Python 端打印
        this.websocket.send(JSON.stringify({
            msg: "Initial Handshake",
            source: "Quest3"
        }));
      };
      ...
      this.websocket.onerror = (event) => {
        ...
        // ❌ 连接失败 -> 变红
        if (statusSphere) statusSphere.setAttribute('color', '#ff0000'); // Red
      };
      ...
      this.websocket.onclose = (event) => {
        ...
        // ⚠️ 断开连接 -> 变黄
        if (statusSphere) statusSphere.setAttribute('color', '#ffff00'); // Yellow
      };

      this.websocket.onmessage = (event) => {
        // console.log(`Msg: ${event.data}`); // 避免刷屏，暂时注释
      };

    } catch (error) {
        console.error(`Failed to create WebSocket:`, error);
        this.reportVRStatus(false);
        if (statusSphere) statusSphere.setAttribute('color', '#ff0000');
    }
    // ==========================================
    // 👆 修改结束 👆
    // ==========================================
...
function addControllerTrackingButton() {
  //document.body.style.backgroundColor = "red"; // 测试代码：如果屏幕变红，说明函数运行了
    if (navigator.xr) {
