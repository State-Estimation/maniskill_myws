...
from maniskill_myws.tasks.sweep_solar_panel import SolarPanelStaticEnv
...
def main(args: Args):
    ...
    while True:
        ...
        if count == 0:
            print(env.unwrapped.agent.robot.get_qpos().cpu().numpy()[0])
            count += 1
...
def solve(env: RecordEpisode, debug=False, vis=False):
